...
%%% Folie
\begin{frame}[allowframebreaks]{Industrielle IoT-Anwendungsfälle}
    \begin{columns}
        \begin{column}[b]{.45\textwidth}
            \begin{center}
...
%%% Folie
\begin{frame}[fragile]{Programmierung: Low Level (AVR)}
    \Justified{
        \footnotesize
...
%%% Folie
\begin{frame}[fragile]{Programmierung: Mid Level (Arduino)}
    \Justified{
        \footnotesize
        Wesentlicher Grund für den Erfolg der Arduino-Plattform ist eigentlich nicht
        die Hardware, sondern dass sie die Programmierung deutlich vereinfacht. Und
        da es inzwischen Arduinos mit ganz unterschiedlichen CPU-Architekturen gibt,
        und auch andere Anbieter die Arduino-Funktionsbibliothek unterstützen, ist
        der Code ganz nebenbei sogar auf andere Hardware-Architekturen übertragbar.
    }
...
\begin{frame}[fragile]{Programmierung: Mid Level (Raspberry Pi)}
    \Justified{
        \footnotesize
        Wenn mehr Rechenleistung als die eines typischen Microcontrollers benötigt
        wird (die Spannweite ist aber auch hier bereits sehr groß), kann auf leistungsfähige
        Hardware wie den Raspberry Pi ausgewichen werden. Diese ermöglicht es uns sogar,
        in einer noch höheren Programmiersprache wie Python auf einem noch höheren
        Abstraktionsniveau zu programmieren.
    }
...
%%% Folie
\begin{frame}[fragile]{Programmierung: High Level (Raspberry Pi)}
    \Justified{
        \footnotesize
        Die Verwendung einer Allzweckprogrammiersprache wie Python unter einem
        Allzweckbetriebssystem wie Linux bringt eine hohe Portierbarkeit der
        eigenen Anwendung mit sich. Im vorherigen Beispiel wird jedoch eine nur
        auf den Raspberry Pi zugeschnittene Bibliothek verwendet, die darüber
        hinaus immer noch relativ low-level aufgebaut ist. Hier sehen wir zum
        Vergleich eine für viele Hardware-Plattformen funktionierende Bibliothek
        mit höherer Abstraktion.
    }
